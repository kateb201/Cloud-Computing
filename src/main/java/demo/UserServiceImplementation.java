...
public class UserServiceImplementation implements UsersService {
    ...
    private boolean assertUser(UserBoundary user) {
        if (user.getName().getFirstname().equals("") || user.getName().getLastname().equals("")) {
    ...
    private UserEntity convertToEntity(UserBoundary boundary) {
        return new UserEntity(boundary.getName().getFirstname(), boundary.getEmail(), boundary.getPassword());
    }

    private UserBoundary convertToBoundary(UserEntity entity) {
        return new UserBoundary(entity.getEmail(), entity.getPassword(), entity.getBirthday());
    }
